...
def load_point_clouds(
    measurements_dir: Union[pathlib.Path, str], threshold: bool, downsample: bool
) -> List:
    """Loads point clouds from the input directory into a single list.

    Args:
        measurements_dir: Directory containing input point clouds.
        threshold: If true, point clouds will be thresholded (subjected to
                   spatial filtering).
        downsample: If true, point clouds will be downsampled.
...
def pairwise_registration(
    source: Type[o3d.cpu.pybind.geometry.PointCloud],
...
def full_registration(
    pcs: List[Type[o3d.cpu.pybind.geometry.PointCloud]],
    max_correspondence_distance_coarse: float,
    max_correspondence_distance_fine: float,
) -> o3d.cpu.pybind.pipelines.registration.PoseGraph:
...
if __name__ == "__main__":
    pcs = load_point_clouds(MEASUREMENTS_DIR, THRESHOLD, DOWNSAMPLE)
    # Perform pairwise registration & build up the pose graph.
    with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm:
        pose_graph = full_registration(
            pcs, MAX_CORRESPONDANCE_DISTANCE_COARSE, MAX_CORRESPONDANCE_DISTANCE_FINE
        )
